...
class ReversePollandNotation
{
    ...
    public int Priority(string path)
    {
        switch (path)
        {
            case "+": return 1;
    ...
    private double DescriptionOperation(string path, params double[] array) => path switch
    {
        "+" => array[0] + array[1],
        "-" => array[0] - array[1],
        "*" => array[0] * array[1],
    ...
    private string GetLabel(ref int position, string expression)
    {
        string output = "";
    ...
    private string GetNumber(ref int position, string expression)
    {
        string output = "";
        for (; position < expression.Length; position++)
        {
...
public class Program
{
    static void Main(string[] args)
    {
        ...
        ReversePollandNotation rpn = new ReversePollandNotation();
        string path = rpn.Transform(joinArgs);
        Console.WriteLine(path);
        double result = rpn.Calculate(path);
